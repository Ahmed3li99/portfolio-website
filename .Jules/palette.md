...
## 2024-05-26 - Hamburger Menu Accessibility
**Learning:** For interactive elements like a custom hamburger menu, ARIA attributes need explicit state management. Even if styling dictates its opened/closed look, screen readers lack this visual context. Therefore, `aria-expanded` must be properly maintained by JavaScript alongside class toggling. Also, the element type (`type="button"`) should be strictly specified to avoid forms misinterpreting it as submit when re-used inside interactive components.
**Action:** When refactoring interactive toggle components like hamburger menus, explicitly include `type="button"`, `aria-controls`, and `aria-expanded` attributes. Ensure the associated JavaScript dynamically updates the `aria-expanded` state on interaction, accurately reflecting the current element state by checking its class presence (e.g., via `classList.contains`).
